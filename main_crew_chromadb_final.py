"""
GPSE Main Crew - Final ChromaDB Fix
Uses temp directory to avoid Windows permission issues
"""
...
# Use temp directory which we know works from our tests
temp_storage = Path(tempfile.gettempdir()) / "gpse_crewai_storage"
temp_storage.mkdir(exist_ok=True)
os.environ['CREWAI_STORAGE_DIR'] = str(temp_storage)

# Set ChromaDB environment variables
os.environ['ALLOW_RESET'] = 'TRUE'
os.environ['ANONYMIZED_TELEMETRY'] = 'FALSE'
# Windows-specific: disable mmap to avoid file locking issues
os.environ['CHROMA_SEGMENT_MANAGER_IMPL'] = 'local'

# Now import CrewAI after setting environment
...
def setup_logging():
    ...
    log_filename = f'logs/gpse_chromadb_final_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
@CrewBase
class GPSECrewChromaFinal():
    """GPSE Crew with final ChromaDB fix"""
    ...
    def __init__(self, inputs: Optional[Dict[str, Any]] = None):
        ...
        logger.info("Initialized GPSE Crew - FINAL CHROMADB FIX")
        logger.info(f"CREWAI_STORAGE_DIR: {os.environ.get('CREWAI_STORAGE_DIR')}")
    
    def _verify_storage_permissions(self):
        """Verify we can write to the storage directory"""
        test_file = temp_storage / "test_permissions.txt"
        try:
            test_file.write_text("Testing permissions")
            test_file.unlink()
            logger.info(f"Storage directory permissions verified: {temp_storage}")
        except Exception as e:
            logger.error(f"Cannot write to storage directory: {e}")
            raise
    ...
    def prepare_analysis(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("=" * 60)
        logger.info("GPSE - FINAL CHROMADB FIX")
        logger.info("=" * 60)
        logger.info(f"Starting at {get_timestamp()}")
        logger.info(f"Using temp storage: {temp_storage}")
...
EXECUTE don't describe!""",
    ...
    @crew
    def crew(self) -> Crew:
        """Crew configuration with final ChromaDB fix"""
        logger.info("Creating crew with final ChromaDB configuration")
        logger.info(f"Using temp storage directory: {temp_storage}")
        
        return Crew(
            agents=self.agents,
            tasks=self.tasks,
            process=Process.sequential,
            memory=True,  # Enable memory with temp storage directory
            embedder={
                "provider": "openai",
                "config": {
    ...
    def _validate_environment(self) -> bool:
        ...
        if not temp_storage.exists():
            issues.append(f"Storage directory does not exist: {temp_storage}")
        elif not os.access(temp_storage, os.W_OK):
            issues.append(f"Storage directory not writable: {temp_storage}")
...
def main():
    """Main execution"""
    try:
        logger.info("Starting GPSE with Final ChromaDB Fix...")
        logger.info(f"Storage directory: {temp_storage}")
        logger.info("This uses temp directory to avoid Windows permission issues")
        
        gpse_crew = GPSECrewChromaFinal()
        ...
        logger.info("=" * 60)
        logger.info("EXECUTING CREW WITH CHROMADB MEMORY...")
        logger.info("All agent enhancements active")
        logger.info("Using temp directory for reliable storage")
        logger.info("=" * 60)
