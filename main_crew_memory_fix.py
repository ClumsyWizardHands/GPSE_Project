"""
GPSE Main Crew - Memory Fix Version
Properly configures ChromaDB to use a local directory with full permissions
"""
...
# Set ChromaDB to use a local directory BEFORE any imports
...
def setup_logging():
    ...
    log_filename = f'logs/gpse_memory_fix_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
@CrewBase
class GPSECrewMemoryFix():
    """GPSE Crew with proper memory configuration"""
    ...
    def __init__(self, inputs: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"ChromaDB directory: {self.chroma_dir.absolute()}")
        ...
        logger.info("Initialized GPSE Crew - MEMORY FIX VERSION")
    ...
    def prepare_analysis(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("=" * 60)
        logger.info("GPSE - MEMORY FIX VERSION")
        logger.info("=" * 60)
        logger.info(f"Starting at {get_timestamp()}")
        logger.info(f"ChromaDB path: {self.chroma_dir.absolute()}")
...
KEEP TOTAL OUTPUT UNDER 2000 WORDS to avoid context issues."""
        ...
        return Agent(
            ...
            max_iter=5,
            memory=True,  # Memory enabled with proper ChromaDB config
            verbose=True,
...
EXECUTE don't describe!""",
    ...
    @crew
    def crew(self) -> Crew:
        """Optimized crew configuration with memory enabled"""
        return Crew(
            agents=self.agents,
            tasks=self.tasks,
    ...
    def _validate_environment(self) -> bool:
        ...
        try:
            test_file = self.chroma_dir / "test_permissions.tmp"
...
def main():
    """Main execution"""
    try:
        logger.info("Starting GPSE Memory Fix...")
        gpse_crew = GPSECrewMemoryFix()
        ...
        logger.info("=" * 60)
        logger.info("EXECUTING MEMORY FIX CREW...")
        logger.info("Memory enabled with local ChromaDB directory")
        logger.info("=" * 60)
