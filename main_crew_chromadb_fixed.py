"""
GPSE Main Crew - Fixed ChromaDB Version
Explicitly sets storage directory to avoid permission issues
"""
...
# Set CREWAI_STORAGE_DIR before importing CrewAI
# This ensures CrewAI uses a directory we control
current_dir = Path(__file__).parent.absolute()
storage_dir = current_dir / "crewai_storage"
os.environ['CREWAI_STORAGE_DIR'] = str(storage_dir)

# Create the storage directory with full permissions
storage_dir.mkdir(exist_ok=True)

# Also set ChromaDB environment variables for good measure
...
def setup_logging():
    ...
    log_filename = f'logs/gpse_chromadb_fixed_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
@CrewBase
class GPSECrewChromaFixed():
    """GPSE Crew with fixed ChromaDB configuration"""
    ...
    def __init__(self, inputs: Optional[Dict[str, Any]] = None):
        ...
        logger.info("Initialized GPSE Crew - FIXED CHROMADB VERSION")
    ...
    def _verify_storage_permissions(self):
        ...
        test_file = storage_dir / "test_permissions.txt"
        try:
            test_file.write_text("Testing permissions")
            test_file.unlink()
            logger.info(f"Storage directory permissions verified: {storage_dir}")
    ...
    def prepare_analysis(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("=" * 60)
        logger.info("GPSE - FIXED CHROMADB VERSION")
        logger.info("=" * 60)
...
EXECUTE don't describe!""",
    ...
    @crew
    def crew(self) -> Crew:
        """Crew configuration with fixed ChromaDB setup"""
        logger.info("Creating crew with fixed ChromaDB configuration")
        ...
        return Crew(
            ...
            memory=True,  # Enable memory with our custom storage directory
            embedder={
                "provider": "openai",
                "config": {
...
def main():
    """Main execution"""
    try:
        logger.info("Starting GPSE with Fixed ChromaDB...")
        logger.info(f"Storage directory: {storage_dir}")
        
        gpse_crew = GPSECrewChromaFixed()
        ...
        logger.info("=" * 60)
        logger.info("EXECUTING CREW WITH FIXED CHROMADB...")
