...
# Try to apply pysqlite3-binary fix if available
try:
    import pysqlite3
    import sys
...
def test_run_as_admin():
    ...
    try:
        ...
        if is_admin:
            ...
            print("   Some permission issues may be resolved by running as Admin")
        return is_admin
    except:
        print("⚠️ Could not determine admin status")
        return False
...
def main():
    ...
    if direct_success:
        ...
        if crewai_success:
            ...
            print(f"- If available, install pysqlite3-binary")
        else:
            print("\n⚠️ ChromaDB works but CrewAI memory still fails")
            print("This confirms it's a CrewAI-specific issue on Windows")
            print("\nRecommendations:")
            print("1. Try running as Administrator (if not already)")
            print("2. Use the no-memory version: python main_crew_no_memory_fixed.py")
            print("3. Check for antivirus interference")
            print("4. Monitor for CrewAI updates that may fix this issue")
    else:
