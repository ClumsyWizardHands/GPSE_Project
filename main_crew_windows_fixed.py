"""
GPSE Main Crew Execution Script - Windows ChromaDB Fixed Version
Applies all Windows-specific fixes for ChromaDB compatibility
"""
...
try:
    ...
    print("⚠️ pysqlite3-binary not installed - ChromaDB may have issues on Windows")
...
from gpse_tools_comprehensive import (
    NewsScoutTool, NewsArchiveTool, WebResearchTool,
    StrategyQuerierTool, DocumentSaverTool, AnalysisArchiveTool
)
...
def create_gpse_crew():
    ...
    # Initialize tools
    news_tool = NewsScoutTool()
    archive_tool = NewsArchiveTool()
    web_tool = WebResearchTool()
    query_tool = StrategyQuerierTool()
    save_tool = DocumentSaverTool()
    analysis_tool = AnalysisArchiveTool()
    ...
    news_scout = Agent(
        ...
        tools=[news_tool, archive_tool, web_tool],
        verbose=True,
    ...
    news_gathering_task = Task(
        description="""Gather comprehensive global political news from the last 24-48 hours.
        
        Requirements:
        1. Query multiple news sources for diverse perspectives
    ...
    analysis_task = Task(
        description="""Produce a comprehensive geopolitical strategic analysis that synthesizes current events 
        with historical patterns and strategic context.
        
        Requirements:
        1. MANDATORY: Query the strategy database for relevant historical analyses
    ...
    documentation_task = Task(
        description="""Format the strategic analysis into a professional document and update the knowledge base.
        
        Requirements:
        1. Create a properly formatted strategic analysis document following the established template
        2. Ensure clear section headers and logical flow
        3. Include metadata:
           - Date and unique identifier
           - Key topics and regions covered
           - Referenced historical analyses
        4. Save the document with proper naming convention: GGSM-MMDDYY-Focus.md
        5. Archive the analysis in the strategy database with appropriate chunks:
           - Executive summary
...
def main():
    ...
    print("Windows ChromaDB Fixed Version")
    print("="*60 + "\n")
    ...
    try:
        ...
        print("\nInitializing GPSE crew with Windows fixes...")
        ...
        print("="*60)
        print(f"Error: {str(e)}")
        print("\nIf you see ChromaDB permission errors, try:")
        print("1. Install pysqlite3-binary: pip install pysqlite3-binary")
        print("2. Run as Administrator (one time to set permissions)")
        print("3. Use the no-memory version: python main_crew_no_memory_fixed.py")
