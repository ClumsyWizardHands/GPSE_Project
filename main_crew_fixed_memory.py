"""
GPSE Main Crew - Fixed Memory Version
Properly configures ChromaDB to work on Windows with environment variables
"""
...
# Force ChromaDB to use our custom directory BEFORE any imports
...
# Critical: Force CrewAI to use our ChromaDB directory
...
def setup_logging():
    ...
    log_filename = f'logs/gpse_fixed_memory_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
def ensure_chromadb_directories():
    """Ensure ChromaDB directories exist with proper permissions"""
    project_root = Path(__file__).parent
    
    # Create dedicated directories for ChromaDB
    chromadb_dirs = [
        project_root / "chromadb_data",
        project_root / "chromadb_data" / "short_term",
        project_root / "chromadb_data" / "long_term",
        project_root / "chromadb_data" / "entity",
    ]
    ...
    return project_root / "chromadb_data"


@CrewBase
class GPSECrewFixedMemory():
    """GPSE Crew with fixed memory configuration"""
    ...
    def __init__(self, inputs: Optional[Dict[str, Any]] = None):
        ...
        self.chromadb_path = ensure_chromadb_directories()
        ...
        logger.info("Initialized GPSE Crew - FIXED MEMORY VERSION")
        logger.info(f"ChromaDB path: {self.chromadb_path}")
        logger.info(f"CHROMA_PERSIST_DIRECTORY set to: {os.environ.get('CHROMA_PERSIST_DIRECTORY')}")
    ...
    def prepare_analysis(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("=" * 60)
        logger.info("GPSE - FIXED MEMORY VERSION")
        logger.info("=" * 60)
        logger.info(f"Starting at {get_timestamp()}")
        logger.info(f"ChromaDB storage: {self.chromadb_path}")
...
EXECUTE don't describe!""",
    ...
    @crew
    def crew(self) -> Crew:
        """Crew configuration with fixed memory"""
        logger.info(f"Creating crew with CHROMA_PERSIST_DIRECTORY: {os.environ.get('CHROMA_PERSIST_DIRECTORY')}")
        
        return Crew(
            agents=self.agents,
            tasks=self.tasks,
            process=Process.sequential,
            memory=True,  # Memory always enabled
            embedder={
                "provider": "openai",
                "config": {
    ...
    def _validate_environment(self) -> bool:
        ...
        try:
            test_file = self.chromadb_path / "test_permissions.tmp"
            test_file.write_text("test")
            test_file.unlink()
            logger.info("ChromaDB directory permissions verified")
        except Exception as e:
            issues.append(f"ChromaDB directory not writable: {e}")
...
def main():
    """Main execution"""
    try:
        logger.info("Starting GPSE with Fixed Memory...")
        logger.info(f"ChromaDB environment variables:")
        logger.info(f"  CHROMA_PERSIST_DIRECTORY: {os.environ.get('CHROMA_PERSIST_DIRECTORY')}")
        logger.info(f"  CHROMA_DB_IMPL: {os.environ.get('CHROMA_DB_IMPL')}")
        
        gpse_crew = GPSECrewFixedMemory()
        ...
        logger.info("=" * 60)
        logger.info("EXECUTING CREW WITH FIXED MEMORY...")
        logger.info("All agent enhancements active")
        logger.info("=" * 60)
