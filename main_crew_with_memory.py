"""
GPSE Main Crew - Memory Enabled Version
Properly configures ChromaDB to work on Windows
"""
...
def setup_logging():
    ...
    log_filename = f'logs/gpse_memory_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
@CrewBase
class GPSECrewWithMemory():
    """GPSE Crew with properly configured memory"""
    ...
    def __init__(self, inputs: Optional[Dict[str, Any]] = None):
        ...
        logger.info("Initialized GPSE Crew - MEMORY ENABLED VERSION")
    ...
    def prepare_analysis(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("=" * 60)
        logger.info("GPSE - MEMORY ENABLED VERSION")
        logger.info("=" * 60)
...
EXECUTE don't describe!""",
    ...
    @crew
    def crew(self) -> Crew:
        """Crew configuration with memory enabled"""
        return Crew(
            agents=self.agents,
            tasks=self.tasks,
...
def main():
    """Main execution"""
    try:
        logger.info("Starting GPSE with Memory...")
        
        gpse_crew = GPSECrewWithMemory()
        ...
        logger.info("=" * 60)
        logger.info("EXECUTING CREW WITH MEMORY ENABLED...")
