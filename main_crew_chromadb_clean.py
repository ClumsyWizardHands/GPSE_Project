"""
GPSE Main Crew - Clean ChromaDB Version
Updated to work with ChromaDB 0.5.x+ without deprecated configurations
"""
...
def setup_logging():
    ...
    log_filename = f'logs/gpse_chromadb_clean_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
@CrewBase
class GPSECrewChromaClean():
    """GPSE Crew with clean ChromaDB configuration"""
    ...
    def __init__(self, inputs: Optional[Dict[str, Any]] = None):
        ...
        logger.info("Initialized GPSE Crew - CLEAN CHROMADB VERSION")
    ...
    def prepare_analysis(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("=" * 60)
        logger.info("GPSE - CLEAN CHROMADB VERSION")
        logger.info("=" * 60)
...
EXECUTE don't describe!""",
    ...
    @crew
    def crew(self) -> Crew:
        """Crew configuration with clean ChromaDB setup"""
        logger.info("Creating crew with clean ChromaDB configuration")
        
        return Crew(
            agents=self.agents,
            tasks=self.tasks,
            process=Process.sequential,
            memory=True,  # Simply enable memory - let CrewAI handle ChromaDB
            embedder={
                "provider": "openai",
                "config": {
            ...
            max_rpm=10,
            share_crew=False
            # No custom ChromaDB configuration - let CrewAI use defaults
        )
...
def main():
    """Main execution"""
    try:
        logger.info("Starting GPSE with Clean ChromaDB...")
        
        gpse_crew = GPSECrewChromaClean()
        ...
        logger.info("=" * 60)
        logger.info("EXECUTING CREW WITH CLEAN CHROMADB...")
